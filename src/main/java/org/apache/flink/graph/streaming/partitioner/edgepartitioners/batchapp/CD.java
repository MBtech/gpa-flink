...
public class CD implements App{
    ...
    public ExecutionEnvironment exec(ExecutionEnvironment env, String[] args) throws Exception {
        ...
        Graph<Long, Long, Double> graph = Graph.fromDataSet(data.partitionCustom(partitioner, new CustomKeySelector<>(0)),new InitVertices(1), env);


        Graph<Long, Long, Double> updatedGraph = graph.run(new CommunityDetection<Long>(maxIterations, 0.5));

        DataSet<Vertex<Long, Long>> result = updatedGraph.getVertices();
