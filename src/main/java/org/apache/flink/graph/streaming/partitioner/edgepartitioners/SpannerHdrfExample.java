...
//import org.apache.flink.graph.streaming.partitioner.tests.DumSink;
...
// */
//public class SpannerHdrfExample implements ProgramDescription {
//
...
//		edges.partitionCustom(new Hdrf.HDRF<>(new CustomKeySelector(0),k,1), new CustomKeySelector<>(0)).writeAsCsv("output", FileSystem.WriteMode.OVERWRITE).setParallelism(k);
//	    edges.aggregate(new Spanner<Long, NullValue>(mergeWindowTime, k)).addSink(new DumSink());
//
...
//										   + "<print window time (ms)> <number of Partitions> <distance factor>");
//				return false;
//			}
//
//			fileOutput = true;
